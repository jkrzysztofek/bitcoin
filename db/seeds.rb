...
def self.profit
  ...
  ***********************
  ...
  maximum_rate = ExchangeRate.minimum.where("rate_date > ? AND rate_date < ?", date_from, date_to) #najmniejszy kurs pomiedzy data_od a data_do podanymi przez uzytkownika
  minimum_rate = ExchangeRate.maximum.where("rate_date > ? AND rate_date < ?", date_from, date_to) #najwiekszy kurs pomiedzy data_od a data_do podanymi przez uzytkownika
...
exchange_rate_with_biggest_price = ExchangeRate.order(rate: :desc).first
exchange_rate_with_lowest_price = ExchangeRate.order(rate: :asc).first

exchange_rate_with_lowest_price_before_biggest = ExchangeRate.where("rate_date > ? AND rate_date < ?").order(rate: :asc).first
exchange_rate_with_biggest_price+after_the_lowest = ExchangeRate.where("rate_date > ? AND rate_date < ?").order(rate: :desc).first
